...
function show(n){if(busy||n<0)return;if(n>=pages.length)n=0;if(n===index)return;var old=pages[index],next=pages[n];if(specialPage(old)||specialPage(next)){index=n;status();return;}busy=true;var forward=n>index;var layer=document.createElement('div');layer.setAttribute('aria-hidden','true');
if(mobile){
  ...
  mleaf.appendChild(mfront);mleaf.appendChild(mback);layer.appendChild(mleaf);book.appendChild(layer);
...
book.addEventListener('touchend',function(e){if(e.target.closest&&e.target.closest('.read-again')){e.preventDefault();start=null;show(0);return;}if(!start)return;var t=e.changedTouches[0],dx=t.clientX-start.x,dy=t.clientY-start.y;start=null;var dir=0;if(Math.abs(dx)>55&&Math.abs(dx)>Math.abs(dy)*1.4)dir=dx<0?1:-1;else if(!mobile&&Math.abs(dx)<20&&Math.abs(dy)<20)dir=direction(e.target);if(dir){e.preventDefault();lastTouch=Date.now();show(index+dir);}},{passive:false});
...
book.addEventListener('click',function(e){if(e.target.closest&&e.target.closest('.read-again')){e.preventDefault();show(0);return;}if(e.target.closest&&e.target.closest('.reader-close'))return;var dir=direction(e.target);if(mobile||!dir)return;e.preventDefault();if(Date.now()-lastTouch>650)show(index+dir);});
