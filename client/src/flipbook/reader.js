...
function titlePageHtml(book, baseUrl) {
  const cover = book.coverUrl || book.pages?.[0]?.imageUrl || '';
  const coverSrc = cover ? esc(safeURL(cover, baseUrl)) : '';
  return `<article class="page title-page current" data-source="title" aria-label="Title page"><div class="title-stage"><div class="title-cover-wrap">${coverSrc?`<img class="title-cover" src="${coverSrc}" alt="">`:''}</div><section class="title-meta"><h1>${esc(book.title||'')}</h1>${book.tagline?`<p class="title-sub">${esc(book.tagline)}</p>`:''}${book.author?`<p class="title-author">${esc(book.author)}</p>`:''}${book.date?`<p class="title-date">${esc(book.date)}</p>`:''}</section></div><button class="zone" data-dir="1" aria-label="Next page"></button></article>`;
}
...
export function createReaderDocument(book,{libraryUrl='/',baseUrl=location.href}={}) {
  ...
  const articles=titlePageHtml(book,baseUrl)+book.pages.map((p,i)=>{
    ...
    const classes=`page${fallback?' facsimile':''}${p.position==='top'?' top-text':''}`;
