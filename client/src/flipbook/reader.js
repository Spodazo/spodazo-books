...
export function createReaderDocument(book,{libraryUrl='/',baseUrl=location.href,credits='',copyright='',logoUrl='',fadeOpen=false,alwaysLandscape=true}={}) {
  ...
  const bootMobile=`(function(){try{var root=document.documentElement;var always=${alwaysLandscape?'true':'false'};if(always)root.classList.add('always-landscape');if(always&&${cover?'false':'true'})root.classList.add('cover-opened');var phone=matchMedia('(max-width:700px), (pointer:coarse) and (max-width:1100px)').matches;var portrait=matchMedia('(orientation:portrait)').matches;var opened=root.classList.contains('cover-opened');var waitCover=always&&phone&&portrait&&${cover?'true':'false'}&&!opened;var m=phone&&portrait&&(!always||waitCover);root.classList.toggle('mobile',m);root.classList.toggle('phone-spread',always&&opened&&phone&&!portrait||(!m&&matchMedia('(orientation:landscape) and (max-height:700px)').matches));if(!m&&${cover?'true':'false'})root.classList.add('closed-book');}catch(e){}})();`;
